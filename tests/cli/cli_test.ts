...
Deno.test("project and module generators create a minimal structure without overwriting targets", async () => {
  ...
  assertEquals(fileSystem.files.has("store-api/src/modules/health/module.ts"), true);
  assertEquals(fileSystem.files.has("store-api/src/modules/users/module.ts"), true);
  assertEquals(fileSystem.files.has("store-api/tests/app_test.ts"), true);
  ...
  assertEquals(
    fileSystem.files.has("store-api/src/modules/order-items/module.ts"),
    true,
  );
  assertEquals(fileSystem.files.has("store-api/tests/modules/order-items_test.ts"), true);
  await assertRejects(
...
Deno.test("module generation requires a project root before writing files", async () => {
  ...
  assertEquals(fileSystem.files.has("outside/src/modules/billing/module.ts"), false);
...
Deno.test("new writes the starter project to a real directory", async () => {
  ...
  try {
    ...
    const stat = await Deno.stat(`${directory}/src/modules/users/module.ts`);
